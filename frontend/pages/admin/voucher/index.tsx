...
const Voucher = () => {
  ...
  const globalVouchersQuery = gql`
    query globalVouchers {
      globalVouchers {
        id
        name
        description
  ...
  const { loading: l, error: e, data: v } = useQuery(globalVouchersQuery);
          ...
          <div className={s.card}>
            ...
            <Card className={s.table}>
              <table>
                ...
                {v?.globalVouchers?.length > 0 ? (
                  v?.globalVouchers.map((p: any) => {
                    return (
                      <tr key={p?.id}>
                        <td className={s.info}>
                          <span>{p?.name}</span>
